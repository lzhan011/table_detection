...
class JAnet(object):
    ...
    def add_validation_env(self,  validation_set ):
        ...
        print("self.FLAGS.validation_dataset_file_path[0]")
        print(self.FLAGS.validation_dataset_file_path[0])
        print("""os.path.splitext(self.FLAGS.validation_dataset_file_path[0])[1] == '.tfrecords'""")
        print(os.path.splitext(self.FLAGS.validation_dataset_file_path[0])[1] == '.tfrecords')
        # exit()
        if os.path.splitext(self.FLAGS.validation_dataset_file_path[0])[1] == '.tfrecords' or True:
...
#        junction_type_loss = tf.nn.softmax_cross_entropy_with_logits_v2(gt_junction_one_hot, heatmaps)
    ...
    def train(self, sess):
        ...
        try:
            ...
            self.occasional_jobs(sess, self.global_step, save=save)
